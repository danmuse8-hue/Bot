...
class FullPredictor:
    ...
    def predict_goals(self, home_team, away_team):
        ...
        # Deterministic exact score using hash
        seed_str = f"{home_team}{away_team}{int(time.time()/60)}"
...
def handle_record(message):
    try:
        ...
        if len(parts) < 3:
            bot.reply_to(message, "❌ Usage: `/record game_id outcome score`\n\nExample: `/record MCI_LIV_1234567890 1 2-1`", parse_mode="Markdown")
            return
